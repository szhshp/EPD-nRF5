...
async function write(cmd, data, withResponse = true) {
  ...
  try {
    if (withResponse)
      ...
      await epdCharacteristic.writeValueWithoutResponse(
...
async function writeImage(data, step = "bw") {
  ...
  for (let i = 0; i < data.length; i += chunkSize) {
    ...
    setStatus(
      `${step == "bw" ? "黑白" : "颜色"}块: ${chunkIdx + 1}/${count + 1}, 总用时: ${currentTime}s`,
    );
    const payload = [
...
async function syncTime(mode) {
  if (mode === 2) {
    if (
      !confirm(
        "提醒：时钟模式目前使用全刷实现，仅供体验，不建议长期开启，是否继续?",
      )
    )
      return;
  }
...
async function sendimg() {
  ...
  // 记录旋转信息用于调试
  addLog(
    `发送图片：原始尺寸 ${imageData.width}x${imageData.height}，旋转后尺寸 ${rotatedImageData.width}x${rotatedImageData.height}`,
  );
...
function downloadDataArray() {
  ...
  // 记录旋转信息用于调试
  addLog(
    `下载数组：原始尺寸 ${imageData.width}x${imageData.height}，旋转后尺寸 ${rotatedImageData.width}x${rotatedImageData.height}`,
  );

  if (
    mode === "sixColor" &&
    processedData.length !== canvas.width * canvas.height
  ) {
    console.log(
      `错误：预期${canvas.width * canvas.height}字节，但得到${processedData.length}字节`,
    );
...
function updateButtonStatus(forceDisabled = false) {
  ...
  document.getElementById("reconnectbutton").disabled =
    gattServer == null || gattServer.connected ? "disabled" : null;
...
function handleNotify(value, idx) {
  ...
  if (idx == 0) {
    ...
    if (msg.startsWith("mtu=") && msg.length > 4) {
      ...
      const t =
        parseInt(msg.substring(2)) + new Date().getTimezoneOffset() * 60;
...
async function connect() {
  ...
  try {
    const versionCharacteristic = await epdService.getCharacteristic(
...
function updateDitcherOptions() {
  document.getElementById("epddriver").selectedIndex = 0;
  document.getElementById("canvasSize").selectedIndex = 0;
  document.getElementById("ditherMode").selectedIndex = 0;
  document.getElementById("ditherAlg").selectedIndex = 0;
  document.getElementById("ditherStrength").value = 0.5;
...
function rotateCanvas() {
  ...
  addLog(
    `画布已旋转: ${currentWidth}x${currentHeight} -> ${canvas.width}x${canvas.height}`,
  );
  updateImage();
}
...
function convertDithering() {
  ...
  const processedData = processImageData(
    ditherImage(imageData, alg, strength, mode),
    mode,
  );
...
function initEventHandlers() {
  document
    .getElementById("epddriver")
    .addEventListener("change", updateDitcherOptions);
  ...
  document
    .getElementById("ditherStrength")
    .addEventListener("input", function () {
      finishCrop();
      document.getElementById("ditherStrengthValue").innerText = parseFloat(
        this.value,
      ).toFixed(1);
    });
  document
    .getElementById("ditherContrast")
    .addEventListener("input", function () {
      finishCrop();
      document.getElementById("ditherContrastValue").innerText = parseFloat(
        this.value,
      ).toFixed(1);
    });
  document
...
document.body.onload = () => {
  ...
  // setTimeout(() => {
  //   /*
  //   1. redirect to debug mode
  //   2. choose the fisrt option from epddriver
  //   3. choose the first option from canvasSize
  //   */
